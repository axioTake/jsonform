...
jsonform.fieldTemplate = function(inner) {
  return '<div class="form-group jsonform-error-<%= keydash %>' +
    ...
    '<div class="controls">' +
      ...
      '<div class="<% if (node.prepend || node.append) { %>input-group<% } %>">' +
        '<% if (node.prepend) { %>' +
          '<div class="input-group-prepend"><span class="input-group-text"><%= node.prepend %></span></div>' +
        '<% } %>' +
      '<% } %>' +
      inner +
      '<% if (node.append) { %>' +
        '<div class="input-group-append"><span class="input-group-text"><%= node.append %></span></div>' +
      '<% } %>' +
      '<% if (node.prepend || node.append) { %>' +
        '</div>' +
      '<% } %>' +
...
var fileDisplayTemplate = '<div class="_jsonform-preview">' +
  ...
  '<% } %>' +
  '</div>' +
  '<a href="#" class="btn btn-secondary _jsonform-delete"><i class="glyphicon glyphicon-remove" title="Remove"></i></a> ';
...
jsonform.elementTypes = {
  ...
  'number': {
    'template': '<div class="form-row"><input type="number" ' +
      'class="form-control-number<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>" ' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      ' min=<%= range.min %>' +
      ' max=<%= range.max %>' +
      ...
      '<div class="btn-group" role="group" aria-label="Spinner">' +
      '<button type="button" class="btn btn-secondary"><i class="glyphicon glyphicon-plus"></i></button>' +
      '<button type="button" class="btn btn-secondary"><i class="glyphicon glyphicon-minus"></i></div></div>',
    'fieldtemplate': true,
    'inputfield': true,
    ...
    'onInsert': function (evt, node) {
      var $buttons = $(node.el).find('button');
      $($buttons.get(0)).on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        // !NOTE: adjust input access when template changes
        this.parentElement.previousElementSibling.stepUp();
      });
      $($buttons.get(1)).on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        // !NOTE: adjust input access when template changes
        this.parentElement.previousElementSibling.stepDown();
      });
    }
  },
  'range': {
    'template': '<input type="range" ' +
      'class="form-control-range<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>" ' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      ' min=<%= range.min %>' +
      ' max=<%= range.max %>' +
  ...
  'checkbox':{
    'template': '<div class="form-check"><input type="checkbox" id="<%= id %>" ' +
      ...
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
      ' /><label class="form-check-label"><%= node.inlinetitle || "" %>' +
      '</label></div>',
    'fieldtemplate': true,
    'inputfield': true,
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<div class="dropdown">' +
      '<a class="btn<% if (buttonClass && node.value) { %> <%= buttonClass %><% } else { %> btn-secondary<% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
  ...
  'radios':{
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %>' +
      '<div class="form-check"><input type="radio" class="form-check-input<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>" ' +
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<div class="form-check"><input type="radio" class="form-check-input<%= (fieldHtmlClass ? " " + fieldHtmlClass: "") %>" style="position:absolute;left:-9999px;" ' +
        '<% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %> name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>" />' +
        '<label class="form-check-label"><%= (key instanceof Object ? key.title : key) %></label></div> ' +
        '<% }); %>' +
      '</div>',
    'fieldtemplate': true,
    'inputfield': true,
  ...
  'array': {
    'template': '<div id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<span class="_jsonform-array-buttons">' +
        '<a href="#" class="btn btn-primary _jsonform-array-addmore" role="button"><i class="glyphicon glyphicon-plus-sign" title="Add new"></i></a> ' +
        '<a href="#" class="btn btn-secondary _jsonform-array-deletelast" role="button"><i class="glyphicon glyphicon-minus-sign" title="Delete last"></i></a>' +
      '</span>' +
      '</div>',
    'fieldtemplate': true,
    'array': true,
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable">' +
      ...
      '</div>' +
      '</div>' +
      '<a href="#" class="btn btn-secondary _jsonform-array-addmore" role="button"><i class="glyphicon glyphicon-plus-sign" title="Add new"></i></a> ' +
      '<a href="#" class="btn btn-secondary _jsonform-array-deleteitem" role="button"><i class="glyphicon glyphicon-minus-sign" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner) {
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child, idx) {
        ...
        tabs += '<li data-idx="' + idx + '"' +
          ((idx === 0) ? ' class="active"' : '') +
          '><a class="draggable tab" data-toggle="tab" role="tab">' +
          escapeHTML(title) +
          '</a></li>';
      });
      data.tabs = tabs;
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="radio<%= (node.formElement.optionsType === "radiobuttons") ? " btn btn-secondary" : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label> <% }); %></div>',
    'fieldtemplate': true,
    'onInsert': function (evt, node) {
      var activeClass = 'active';
      var elt = node.formElement || {};
